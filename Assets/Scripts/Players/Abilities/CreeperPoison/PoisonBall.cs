...
public class PoisonBall : Skill, IAltAbility
{
    ...
    protected override IEnumerator PrepareJob(Action<TargetInfo> callbackDataSaved)
    {
        ...
        while (GetTarget() == null && float.IsPositiveInfinity(_firstMousePosition.x))
        {
            if (GetMouseButton)
            {
                FindTarget(true);
    ...
    private void CheckWhoTarget()
    {
        if (GetTarget() != null)
        {
            if (GetTarget().gameObject == _player.gameObject)
            {
            ...
            else if (GetTarget().gameObject.layer == LayerMask.NameToLayer("Allies"))
            {
            ...
            else if (GetTarget().gameObject.layer == LayerMask.NameToLayer("Enemy"))
            {
                _poisonBallInfo.IsOriginalTargetPlayer = false;
                _poisonBallInfo.IsOriginalTargetAllies = false;
    ...
    private bool CheckCanCast()
    {
        ...
        return Vector3.Distance(_firstMousePosition, transform.position) <= Radius &&
            ...
            NoObstacles(GetTarget().transform.position, _obstacle);
    ...
    {
        if (_isTarget && GetTarget().gameObject != _player.gameObject)
        {
            _isFast = Vector3.Distance(_player.transform.position, _secondMousePosition) > Vector3.Distance(_player.transform.position, GetTarget().transform.position);
        }
        else
        {
    ...
    private void ChooseWhichProjectileCreate()
    {
        if (_isTarget)
        {
            CmdCreateProjectileForTarget(GetTarget().gameObject, GetTarget().transform.position,
    ...
    private void CreateArrowsParallelToPlayer()
    {
        ...
        Vector3 center = GetTarget() != null ? GetTarget().transform.position : _firstMousePosition;
        center.y = 0.8f;
