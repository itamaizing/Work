...
public class AbilityManager : MonoBehaviour
{
	...
	public void AddAbilityToQueue(AbilityBase ability)
	{
		...
		if (abilityQueue.Count == 1 || abilityQueueAutoattack.Count == 1) // ≈сли это перва€ способность в очереди, начните ее выполнение
		{
			ExecuteNextAbility();
		}
	}
	...
	private void SwapAutoattacks()
	{
		if (abilityQueueAutoattack.Count > 1)
		{
			...
			{
				if (abilityQueueAutoattack[0].isInRadius) // перва€ автоатака дот€гиваетс€
				{
				...
				else if (abilityQueueAutoattack[0].Distance < abilityQueueAutoattack[1].Distance) // перва€ автоатака не дот€гиваетс€ и ее дальность < второй
				{
	...
	private void SwapAutoattacksVisualisation() // костыль - отображение круга, отдельно чтобы можно было мен€ть во врем€ паузы (когда есть способность и выбрана цель)
	{
		if (abilityQueueAutoattack.Count > 1)
		{
			if (abilityQueueAutoattack[0].TargetParent == abilityQueueAutoattack[1].TargetParent)
	...
	private void ExecuteNextAbility()
	{
		...
		{
			if (abilityQueue[0].TargetParent != null) // если выбрали цель дл€ способности, и есть автоатака, останавливаем атаку
			{
